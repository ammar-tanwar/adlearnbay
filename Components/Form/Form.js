...
const Form = ({
  ...
  const formSubmit = (e) => {
    e.preventDefault();
    if (HideInterest === false || HideInterest === undefined) {
      if (
        query.interstedin === "" ||
        ...
        if (
          ...
          (router.pathname === "/scholarship" && downloadBrochure) ||
          (router.pathname === "/offer" && downloadBrochure) ||
  ...
  const formSubmitDownload = (e) => {
    e.preventDefault();
    if (CTC) {
      if (query.CTC === "" || query.CTC === "CTC") {
        setError(true);
        console.log("inside CTC");
      } else {
        setError(false);
        console.log(query);
        const formData = new FormData();
